...
class Model(object):
    ...
    def __init__(self):
        ...
        self.y_mean = 33.173
        self.y_std = 18.514
        self.x_mean = np.array([0.4731, 0.5726])
        self.x_std = np.array([0.28076, 0.27753])
    ...
    def fitting_model(self, train_GT: np.ndarray,train_features: np.ndarray):
        ...
        for clu in range(self.n_experts):
    ...
    def cluster(self, X):
        self.kmeans = KMeans(n_clusters=self.n_experts, random_state=0).fit(X)
